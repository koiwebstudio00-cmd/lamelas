...
4. **Modelo:** enums `operacion` = `venta | alquiler`, `tipo` = `monoambiente | departamento | casa | duplex | local_comercial | oficina | galpon | estacionamiento | terreno | otro`, `moneda` = `ARS | USD`. Las de alquiler traen además `requisitos`, `destino`, `plazo_contrato` (`meses_12..36`/`otro`) + `plazo_otro`, `ajuste` + `ajuste_otro`, `indice_ajuste` (`icl|ipc|fijo`) + `indice_fijo_pct`, `expensas`, `mascotas`, `amoblado`, y `lat`/`lng`/`link_maps` para el mapa (Leaflet). Los labels de los enums viven en `src/types.ts`.
5. **Solo `titulo`, `operacion`, `tipo` y `precio` son obligatorios.** La UI debe tolerar null en dormitorios, baños, superficies, zona, ciudad, descripción, campos de alquiler, mapa y fotos. El select de `/export` ya excluye `notas`, `user_id` y `tenant_id`: no hay riesgo de fuga, pero tampoco hay que pedirlos.
